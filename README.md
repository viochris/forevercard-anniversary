...
In the interest of accurate documentation, a couple of components exist in the codebase but are not currently wired into the live app.

- **`AnniversaryCounter.tsx`** and **`CountdownTimer.tsx`** are both defined but never imported or rendered anywhere in `App.tsx` or any screen component. The `daysTogether` and `anniversaryBadgeText` fields in `cardData.ts` are likewise defined but not currently displayed anywhere, so the "1 Year, 365 Days" style counter described in the project's original design is not yet visible in the live app.
- The project's `package.json` lists `@google/genai` and `jspdf` as dependencies, but neither is actually imported or used anywhere in the current source, they appear to be leftovers from earlier iterations of the build.

These are documented here rather than silently glossed over, and would be reasonable next steps if the project is revisited.
